var EpisodeMixin = {
    ...
    streamCreated: function(e){
      ...
      self.guests_in_line = $.unique(self.guests_in_line);
      console.log("add stream:"+self.guests_in_line);
      if(self.subclassName == "Backstage"){
        CSEventManager.broadcast("UPDATE_MANAGER");
    ...
    streamDestroyed: function(e){
      ...
      for(var i=0; i < self.guests_published.length; i++) {
        if( self.guests_published[i].id == e.stream.id ){
          ...
          // console.log("remove stream[published]:"+e.stream.id);
          CSEventManager.broadcast("UPDATE_MANAGER");
        }
      }
      if(self.subclassName == "Backstage"){
    ...
    connectRemoteStreamHandler: function( data ){
      ...
      this.connectToRemoteStream(data.streamId, data.elementId, {width: data.width, height:data.height});
    },

    connectToRemoteStream: function( streamId, elementId, options ){
      ...
      if(stream){
        ...
        self.session.subscribe(stream, elementId, streamOptions); 
      }
    },
